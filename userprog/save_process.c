...
process_create_initd (const char *file_name) {
	...
	tid = thread_create (retptr, PRI_DEFAULT, initd, fn_copy);
...
duplicate_pte (uint64_t *pte, void *va, void *aux) {
	 ...
	 *    permission. */
	//? current->pml4 의 va 주소에 newpage를 writable 상태로 할당한다.
...
process_exit (void) {
	...
	for(int idx = t->next_fd; idx >= 0; idx--) {
		if(process_get_file(idx) && t->fd_table[idx] != t->exec_file){
			file_close(t->fd_table[idx]);
		}
	}
...
void argument_stack(char **parse, int count, void **esp) {
	...
	**(int64_t **)esp = NULL;
    ...
    for( i = count - 1; i > -1; i--) {
        *esp = (int64_t*)*esp - 1;
        **(int64_t **)esp = save_parse[i];
    }
    //? "문자열을 가리키는 주소들의 배열"을 가리킴
	save_parse[0] = *esp;
	// 이후, tf.R.rsi 에 적용
    //? "문자열의 개수"를 저장
	// 이후, tf.R.rdi 에 count 적용
    //? "Fake Address(0)"을 저장
    *esp = (int64_t*)*esp - 1;
    **(int64_t **)esp = NULL;
...
struct thread *get_child_process(int pid) {
	...
	for (struct list_elem* e = list_begin(&curr->child_processes); e != list_end(&curr->child_processes);) {
